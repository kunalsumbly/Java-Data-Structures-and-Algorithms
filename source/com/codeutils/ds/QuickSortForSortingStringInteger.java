...
public class QuickSortForSortingStringInteger {
	public static void main(String[] args) {
		// String[] str_array = { "8", "9", "3", "4", "-1", "0" };
		String[] str_array = { "8", "9", "3", "4", "-1", "0", "4", "55", "7", "2", "1", "6", "-5" };
		//String[] str_array = { "1", "-1", "0" };
	...
	static String[] quickSort(String[] a) {
		...
		if (a.length < 2) { // base case 1
			...
			if (a.length == 2) { // only 2 elements left in the array base case 2
				...
				if (Integer.valueOf(pivot) < Integer.valueOf(a[1])) {
				...
				less_arr = new String[a.length - 1]; // also removing pivot from here
				greater_arr = new String[a.length - 1]; // also removing pivot from here

				// fill the less_arr
				int i = 0;

				for (String l_str : a) {
					if (l_str != null && Integer.valueOf(pivot) > Integer.valueOf(l_str)) {
						less_arr[i] = l_str;
						i++;
					} else {
						continue;
